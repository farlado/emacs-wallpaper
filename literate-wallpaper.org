...
** Dummy ~feh~ command
   :properties:
   :header-args: :tangle "test/feh"
   :end:

   This is a basic placeholder so that we don't trigger a warning when loading the
   package in a batch environment.
...
** Set =PATH= for ~feh~ dummy

   This prevents a warning when loading the package in a testing environment
   which may not have a proper graphical environment attached to it, much less
   an executable named ~feh~ within =PATH=.
